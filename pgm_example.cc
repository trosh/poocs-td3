...
int main()
{
  ...
  std::cout << "Image " << img.width() << " x " << img.height() << " by " << (img.maxval()+1) << " values\n";
  std::cout << "Stats:\n";
  const unsigned fullsize = img.height()*img.width();
  for(int i=0;i<statsize;++i)
    {
      // Réflexion: et si on met la multiplication par 100 à la fin, qu'obtient-on ? (et pourquoi)
      const unsigned linesize = 100*stats[i]/fullsize;
      std::cout << std::setw(3) << linesize << "% : "; // setw permet ici d'aligner l'écriture sur 3 caractères
